...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if ($http_code === 200) {
        ...
        $message = '❌ Error al eliminar el servicio: ' . ($responseData['message'] ?? 'Respuesta inesperada de la API.');
...
if ($http_code === 200) {
    ...
    if (!is_array($servicios)) {
        ...
        $message = '⚠️ No se encontraron servicios o la respuesta de la API no es válida.';
        $message_type = 'alert-warning';
    }
} else {
    $message = '❌ Error al cargar servicios: ' . ($response ? json_decode($response, true)['message'] ?? 'Error desconocido de la API.' : 'Error de conexión.');
    $message_type = 'alert-danger';
}
?>
...
<body>
    <div class="p-3 navbar-color d-flex justify-content-between align-items-center">
        ...
        <a href="nuevo_servicio.php" class="btn btn-success">➕ Añadir servicio</a>
    </div>
